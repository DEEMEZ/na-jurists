...
| `DATABASE_URL` | Yes | Prisma connection string. Dev: `file:./dev.db` (SQLite). Production: PostgreSQL, e.g. `postgresql://user:pass@host:5432/dbname`. |
...
- **First deploy:** run migrations before starting the server. Optionally run `npx prisma db seed` once if you need the default admin/client demo users (see `prisma/seed.ts`).
- **Production DB:** switch `DATABASE_URL` to PostgreSQL and run the same `prisma migrate deploy` against that database.
...
- **Backend:** `npm ci` → `prisma migrate deploy` → `prisma db seed` → `npm run lint` → `npm test` → `npm run build`
