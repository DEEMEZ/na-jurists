...
- **Supabase `seed:admin` (2026-03-28):** If Node reports missing `SUPABASE_SERVICE_ROLE_KEY` while the editor shows a JWT, the on-disk `scripts/seed-admin.env` may still have an empty value — **save the file** before `npm run seed:admin`. Service role keys pasted in chat should be **rotated** in Supabase.
- **Admin user email validation (2026-03-31, Executor):** Shared Zod `emailField` in `backend/src/lib/userEmail.ts` (trim, `.email()`, lowercase) for `POST/PATCH /api/v1/admin/users`. Frontend `AdminUsersPage` uses `parseValidEmail` before submit. Supabase edge `portal-admin-users` uses matching `normalizeValidEmail` on create and email update. Restored accidental overwrite of `backend/src/lib/email.ts` (SMTP `sendMail`); SMTP module stays `email.ts`, validation is `userEmail.ts`.
- **Portal perf (2026-03-31, Executor):** `portalApi` `requireProfile` short TTL cache + clear on auth events / `invalidatePortalProfileCache` on logout. Client/admin dashboard Supabase queries batched with `Promise.all`. `CaseDetailPage` loads case, messages, and (admin) clients in parallel. Route-level `React.lazy` for protected pages + `Suspense` in `PortalLayout`. First paint after login can feel faster (smaller initial JS); navigation loads page chunks on demand.
- **Public website cases filters (2026-04-10, Executor):** `CasesFilter` now takes `filterValues` from parent and syncs via `useEffect` so court/subject/search UI matches URL + parent state after pagination and after the loading spinner remounts (`src/app/cases/page.tsx`, `HomeCases.tsx`, `CasesFilterProps` in `LegalCase.ts`). **Planner/human:** please confirm in browser that filters stay selected when changing pages and after refresh with `?court=` / `?subject=` in the URL.

**Issues Legal Web App 10-4-26.docx (text extracted 2026-04-10):** Issue 1 — client case list title doesn’t match admin-assigned case name (detail view correct). Issue 2 — two hearing-related controls behave differently (one opens case list, one opens case); stakeholder wants a hearings list page like notifications + link to related case. Issue 3 — email delivery for notifications/messages/hearings. Issue 4 — “display on website” option exists but cases not showing on public site. (Screenshots in the .docx are not in the extracted text; use the Word file for visuals.)

**Executor implementation (2026-04-10):** (1) **Client My matters list (Supabase):** `GET /api/v1/me/cases` in `portalApi.ts` now loads `case_id` from assignments then `cases.select(...).in('id', …)` so reference/title/status match detail (embedded join was returning empty).

**Executor follow-up (2026-04-10):** **My matters** still showed `"Matter"` / `—` because plain `cases` per-id `maybeSingle()` returned no row under RLS while case detail worked via the **assignment + `cases!case_assignments_case_id_fkey` embed** fallback. `GET /api/v1/me/cases` now uses that embed in the list query first, then direct `cases` only for IDs still missing — aligned with `me/cases/:id` behavior.

**Root cause (2026-04-10):** Console `[me/cases] no case row for assignment <uuid>` with both embed and direct fetch empty: **`cases_select_assigned_or_admin` RLS bug** — inside `EXISTS`, `ca.case_id = id` bound `id` to **`case_assignments.id`**, not **`cases.id`**, so the policy never allowed assigned clients to read `public.cases`. **Fix:** migration `20260411120000_fix_cases_select_rls_qualify_case_id.sql` replaces with `ca.case_id = public.cases.id`. Apply via `supabase db push` or paste SQL in Supabase Dashboard → SQL Editor, then reload the app. (2) **Hearings:** Client `/hearings` page + `GET /api/v1/me/hearings`; dashboard “Upcoming hearings” card links to `/hearings`; upcoming-hearing rows use “All hearings” → `/hearings`; list has “View related matter” → case. Nav + dashboard chip. (3) **Email:** `supabase/functions/portal-notify-email` (Resend) + `notifyClientByEmail` after status notifications, new hearing (clients), admin message (clients); deploy + `RESEND_API_KEY` / from-address secrets documented in `SUPABASE.md`. (4) **Public site:** `src/app/api/cases/route.ts` — `dynamic = 'force-dynamic'`, dev warning if service key missing, log Supabase errors, shorter CDN cache.

**Branding / favicon (2026-04-10, Executor):** Header/footer on both site and portal already use `/text-logo.png` (`Navbar`, `PortalLogo`, `PortalFooter`). Stakeholder asked for the **circular N&A emblem** (same as site) on the portal—especially visible in the **browser tab**. Added **`public/emblem-icon.png`** and **`law-firm-portal/frontend/public/emblem-icon.png`** (32×32 PNG, emblem cropped from `text-logo.png` ~22% width). **Favicon:** `src/app/layout.tsx` and `law-firm-portal/frontend/index.html` now point `<link rel="icon">` to `/emblem-icon.png`. **Planner/human:** Hard-refresh or clear site data if the old tab icon is cached; if the crop trims too much/little, adjust the crop ratio in a small script and regenerate.
...
- **`seed-admin.env`:** Node reads the saved file on disk, not unsaved editor buffers; empty `SUPABASE_SERVICE_ROLE_KEY=` on disk causes seed to fail until the file is saved.
- **Postgres RLS:** In `EXISTS (SELECT … FROM case_assignments ca WHERE ca.case_id = id)`, unqualified `id` can resolve to **`ca.id`**, not the secured row’s id. Qualify as **`public.cases.id`** (or the correct table name) on policies for `public.cases`.
